...
import { Reflector, type ReflectionResult } from './reflector.js';
import type { ToolResult } from '../../types/index.js';

export type AgentLoopPhase = 'PLAN' | 'ACT' | 'REFLECT';
...
export interface LoopStepResult {
  ...
  toolResults?: Array<{ toolName: string; args: any; result: ToolResult }>;
...
export class AgentLoop {
  ...
  public async run(
    ...
    getToolCallsFromLlm: (prompt: string) => Promise<LlmToolCall[]>,
    onStepUpdate?: (stepResult: LoopStepResult) => void
    ...
    const executionHistory: Array<{ toolName: string; args: any; result: ToolResult }> = [];
    let currentFeedback = '';
    ...
    for (let step = 0; step < safeMaxSteps; step++) {
      ...
      if (toolCalls.length === 0) {
        const reflection = await this.reflector.reflectSemantically(
          goal,
          executionHistory,
          plan,
          llmCall,
          'The ACT phase returned no tool calls.'
        );
        ...
        if (reflection.decision === 'adjust') {
          plan = await this.planner.generatePlan(
            goal,
            `${context}\nAdjustment feedback: ${reflection.feedback || reflection.reason}`,
            llmCall
          );
        }
      ...
      const stepResults: Array<{ toolName: string; args: any; result: ToolResult }> = [];
  ...
  private buildActPrompt(
    goal: string,
    plan: string[],
    executionHistory: Array<{ toolName: string; args: any; result: ToolResult }>,
    feedback: string
  ): string {
    const history = executionHistory.length > 0
      ? executionHistory
          .map(
            (h, i) =>
              `Step ${i + 1}: ${h.toolName} -> success: ${h.result.success}, output: ${h.result.output.slice(
                0,
                500
              )}${h.result.error ? `, error: ${h.result.error}` : ''}`
          )
          .join('\n')
