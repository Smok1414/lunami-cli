...
import { Executor, type LlmToolCall } from './executor.js';
import { Reflector } from './reflector.js';
import { ToolRegistry } from '../tools/registry.js';
import type { AgentRunOptions, AgentEvent, ToolResult } from '../../types/index.js';
import { streamComplete, type LLMResponse, systemPrompt } from '../../llm.js';
...
export interface LunaticMemory {
  history: MemoryEntry[];
}
...
export class LunaticEngine {
  ...
  public async run(options: AgentRunOptions): Promise<ChatCompletionMessageParam[]> {
    ...
    const dryRun = (options as any).dryRun === true;
    ...
    const virtualFiles: Record<string, string> = {}; // for dry-run mode
        ...
        if (isWrite) {
          ...
          if (filePath) {
            ...
            if (toolCall.name === 'writeFile' || toolCall.name === 'file.write') {
              ...
              const patches = toolCall.arguments.patches as any[];
      ...
      if (modifiedFiles.size > 0 && !dryRun) {
        ...
        if (!validRes.valid) {
          ...
          for (let attempt = 1; attempt <= 2; attempt++) {
            ...
            let healResponse: LLMResponse;
            try {
              healResponse = await streamComplete(
                messages,
                {
                  onTextDelta: (delta) => {
                    if (!healStreamedText) {
  ...
  private async verify(modifiedFiles: Set<string>): Promise<{ valid: boolean; error?: string }> {
  ...
  private createStreamId(prefix: string): string {
    return `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  }

  private async rollback(originalContents: Record<string, string | null>): Promise<void> {
  ...
  private async saveSnapshot(task: string, originalContents: Record<string, string | null>): Promise<void> {
    try {
  ...
  private async loadMemory(): Promise<LunaticMemory> {
    try {
      const memoryPath = resolve(getCwd(), '.lunatic', 'memory.json');
      const text = await readFileNode(memoryPath, 'utf8');
      return JSON.parse(text) as LunaticMemory;
    } catch {
      return { history: [] };
    }
  }

  private async saveMemoryEntry(task: string, success: boolean, modifiedFiles: string[], errors: string[]): Promise<void> {
    try {
  ...
  private buildMemoryPrompt(memory: LunaticMemory): string {
...
${items}`;
  ...
  private countDiffLines(oldContent: string, newContent: string) {
