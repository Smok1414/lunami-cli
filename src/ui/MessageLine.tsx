...
export function MessageLine({
    ...
    return (
      <Box flexDirection="column" marginBottom={1}>
        {lines.map((line, index) => (
          <Text key={`${message.id}-${index}`} color={theme.user} wrap="truncate-end">
            {index === 0 ? '▸ ' : '  '}
  ...
  if (message.kind === 'error') {
    ...
    return (
      <Box flexDirection="column" marginBottom={1}>
        {lines.map((line, index) => (
          <Text key={`${message.id}-${index}`} color={theme.error} wrap="truncate-end">
            {index === 0 ? '! ' : '  '}
            {index === 0 ? timestamp : null}
            {line}
          </Text>
  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      {lines.slice(0, displayCount).map((line, index) => {
        ...
        return (
          <Text key={`${message.id}-${index}`} color={color} wrap="truncate-end">
            {prefix}
            {index === 0 ? timestamp : null}
            {gutter}
            {diff.body}
      ...
      {segments.map((segment, segIndex) => {
          ...
          return (
            ...
            >
              {codeLines.map((line, index) => {
                ...
                return (
                  <Text key={`${message.id}-c-${segIndex}-${index}`} color={theme.code} wrap="truncate-end">
                    {showMeta ? (
                      <>
                        ◆ {timestamp}
                      </>
        ...
        return (
          <React.Fragment key={`${message.id}-txt-${segIndex}`}>
            {lines.map((line, index) => (
              <Text key={`${message.id}-t-${segIndex}-${index}`} color={theme.assistant} wrap="truncate-end">
