...
export function CommandPalette({commands: items, selectedIndex, query, theme}: CommandPaletteProps): React.ReactElement | null {
  ...
  return (
    <Box flexDirection="column" flexShrink={0} borderStyle={borderStyle} borderColor={theme.inputBorder} paddingX={1} marginBottom={1}>
        ...
        return (
          <Box key={group} flexDirection="column">
            <Text color={theme.accent} bold>
              {asciiFallback(`─ ${groupTitle[group]()} ─`, `- ${groupTitle[group]()} -`)}
            </Text>
            {groupItems.map((cmd) => {
