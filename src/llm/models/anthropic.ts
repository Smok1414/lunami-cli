...
export class AnthropicProvider implements LLMProvider {
  ...
  async chat(messages: LLMMessage[], tools: LLMTool[] = []): Promise<LLMResponse> {
    ...
    const response = await this.client.messages.create({
      ...
      tools: tools.length > 0 ? (this.toAnthropicTools(tools) as Anthropic.Tool[]) : undefined
    });
  ...
  async streamChat(
    ...
    const stream = this.client.messages.stream({
      ...
      system: request.system,
      messages: request.messages as Anthropic.MessageParam[],
      tools: requestTools.length > 0 ? (this.toAnthropicTools(requestTools) as Anthropic.Tool[]) : undefined
    });
  ...
  private toAnthropicRequest(messages: LLMMessage[]): AnthropicRequest {
    const system: string[] = [];
  ...
  private toAnthropicTools(tools: LLMTool[]): unknown[] {
  ...
  private getAnthropicText(content: unknown[]): string {
    return content
      .map((block) => {
  ...
  private getAnthropicToolCalls(content: unknown[]): LlmToolCall[] {
  ...
  private parseToolInput(rawArguments: string): Record<string, unknown> {
    try {
