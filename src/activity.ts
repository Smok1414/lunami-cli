...
export type ActivityStepKind = 'thought' | 'work' | 'planning';
...
export function createEmptyToolCounts(): ToolActivityCounts {
  return {
    readFile: 0,
    tree: 0,
    search: 0,
...
export function formatWorkLabel(counts: ToolActivityCounts): string {
  const parts: string[] = [];
...
export function formatStepLine(step: ActivityStep): string {
...
export function finishActivityStep(steps: ActivityStep[], id: string, startedAt: number): void {
