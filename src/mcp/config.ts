...
export function getMcpConfigPaths(): { workspace: string; global: string } {
  return {
...
export async function readMcpConfigFile(path: string): Promise<McpConfigFile | null> {
  try {
...
export function parseMcpConfigJson(raw: string, sourceLabel = 'mcp.json'): McpConfigFile {
  let parsed: unknown;
...
export function mergeMcpConfigs(
  globalConfig: McpConfigFile | null,
  workspaceConfig: McpConfigFile | null
): McpConfigFile {
  return {
    mcpServers: {
...
export async function loadMergedMcpConfig(): Promise<{
  config: McpConfigFile;
  paths: { workspace: string; global: string };
  loadedFrom: { workspace: boolean; global: boolean };
}> {
...
function normalizeServerConfig(value: unknown, name: string, sourceLabel: string): McpServerConfig {
