...
export class OpenAIProvider implements LLMProvider {
  ...
  async chat(messages: LLMMessage[], tools: LLMTool[] = []): Promise<LLMResponse> {
    ...
    const safeRawToolCalls: ChatCompletionMessageToolCall[] = toolCalls.map((tc) => ({
      id: tc.id,
      type: 'function',
      function: {
        name: tc.name,
  ...
  async streamChat(
    ...
    const rawToolCalls: ChatCompletionMessageToolCall[] = [];
...
function normalizeToolCalls(toolCalls: ChatCompletionMessageToolCall[]): LlmToolCall[] {
...
function parseToolArguments(rawArguments: string): Record<string, unknown> {
  try {
    ...
    // Some models/proxies double-encode arguments as a JSON string
