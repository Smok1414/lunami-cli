// File: src/tools/file/generate.tool.ts
...
export type GenerateProjectType = 'react' | 'node' | 'next' | 'python';

export interface GenerateProjectPayload {
  ok: true;
  name: string;
  type: GenerateProjectType;
  root: string;
  files: string[];
...
export function formatScaffoldRunInstructions(projectRoot: string, type: GenerateProjectType): string {
  if (type === 'python') {
...
export class GenerateProjectTool implements ITool {
  public readonly name = 'generateProject';
  public readonly description = 'Generate a small working project scaffold in the current workspace using only filesystem writes.';
  public readonly parameters = {
    ...
    properties: {
      name: {
        type: 'string',
        description: 'Single folder name for the project.'
      },
      type: {
        type: 'string',
    ...
    try {
      ...
      if (typeof name !== 'string' || typeof type !== 'string') {
        return {
          success: false,
          output: '',
          error: 'Arguments "name" and "type" must be strings.'
        };
      }

      validateProjectType(type);
      const payload = await this.generate(name, type);
  ...
  public async generate(name: string, type: GenerateProjectType): Promise<GenerateProjectPayload> {
...
function scaffoldReadmeRunSection(name: string, type: GenerateProjectType): string {
...
function validateProjectType(type: string): asserts type is GenerateProjectType {
  if (!['react', 'node', 'next', 'python'].includes(type)) {
    throw new Error(`Unsupported project type: ${type}`);
  }
}

function getTemplateFiles(name: string, type: GenerateProjectType): TemplateFile[] {
  switch (type) {
    case 'react':
...
function createReactTemplate(name: string): TemplateFile[] {
  return [
    {
      path: 'package.json',
...
function createNodeTemplate(name: string): TemplateFile[] {
  return [
    {
      path: 'package.json',
      content: `${JSON.stringify({
...
function createNextTemplate(name: string): TemplateFile[] {
  return [
    {
      path: 'package.json',
      content: `${JSON.stringify({
...
function createPythonTemplate(name: string): TemplateFile[] {
  return [
    {
      path: 'main.py',
