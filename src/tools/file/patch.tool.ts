...
export class PatchTool implements ITool {
  ...
  public readonly description = 'Patch a file by replacing specific line ranges with new content. Useful for precise and partial edits.';
  ...
  private createMiniDiff(path: string, oldContent: string, newContent: string): string[] {
    ...
    return this.appendTruncation(
      diffLines,
      Math.max(0, removedBlock.length - removed.length) + Math.max(0, addedBlock.length - added.length)
    );
  }

  private appendTruncation(lines: string[], hiddenCount: number): string[] {
    if (hiddenCount > 0) {
