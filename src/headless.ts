...
const green  = (t: string, nc: boolean) => c("32", t, nc);
const yellow = (t: string, nc: boolean) => c("33", t, nc);
const red    = (t: string, nc: boolean) => c("31", t, nc);
const cyan   = (t: string, nc: boolean) => c("36", t, nc);
const gray   = (t: string, nc: boolean) => c("90", t, nc);
const bold   = (t: string, nc: boolean) => c("1",  t, nc);
...
class HeadlessReporter {
  ...
  public filesChanged: Set<string> = new Set();
  public rounds = 0;
  private debugLog?: fs.WriteStream;

  // Track the current round manually based on tool_start events
  private currentRound = 0;
  private currentToolStartTime = 0;
  private currentToolName = '';

  constructor(opts: { noColor: boolean; verbose: boolean; quiet: boolean; debug: boolean; json: boolean }) {
  ...
  handleEvent(event: AgentEvent) {
    if (event.type === 'tool_start') {
      ...
      if (this.verbose) {
        process.stderr.write(
          `${cyan("⚙", this.nc)} round ${this.rounds}  ${yellow(event.name, this.nc)}  ${gray(event.summary.split('\n')[0], this.nc)}\n`
        );
      } else {
        process.stderr.write(
          `${cyan("⠸", this.nc)} ${gray(`round ${this.rounds} · ${event.name}`, this.nc)}\r`
        );
      }
    ...
    if (event.type === 'tool') {
      ...
      if (isError && this.verbose) {
        process.stderr.write(
          `${red("✗", this.nc)} ${event.name} ${red("failed", this.nc)}: ${gray(event.summary.split('\n')[0], this.nc)} ${gray(`(${ms}ms)`, this.nc)}\n`
        );
      } else if (this.verbose) {
        process.stderr.write(
          `${green("✓", this.nc)} ${event.name} ${green("success", this.nc)}: ${gray(event.summary.split('\n')[0], this.nc)} ${gray(`(${ms}ms)`, this.nc)}\n`
        );
      }
  ...
  success(summary: string): HeadlessResult {
    const ms = Date.now() - this.startTime;
    process.stderr.write(
  ...
  failure(error: string, exitCode = 1, muteLog = false): HeadlessResult {
    const ms = Date.now() - this.startTime;
    if (!muteLog) {
      process.stderr.write(
    ...
    process.stderr.write(
      `${gray(`exit: `, this.nc)}${red("✗", this.nc)} ${gray(t('headless_stats', this.rounds, this.filesChanged.size, (ms / 1000).toFixed(1)), this.nc)}\n`
    );
  ...
  cancelled(summary = t('headless_interrupted')): HeadlessResult {
    const ms = Date.now() - this.startTime;
...
export async function runHeadless(opts: HeadlessOptions): Promise<HeadlessResult> {
  ...
  let rejectOnInterrupt: ((reason: Error) => void) | undefined;
