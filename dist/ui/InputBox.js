...
export function InputBox({ typedText, pastedText, boxWidth, theme, agentMode, placeholder }) {
    ...
    const previewLines = pastedText
        ...
        : [];
    return (_jsxs(Box, { flexDirection: "column", width: fieldWidth, marginBottom: 1, flexShrink: 0, children: [previewLines.length > 0 ? (_jsxs(Box, { flexDirection: "column", borderStyle: "single", borderColor: theme.inputBorder, paddingX: 1, marginBottom: 1, width: fieldWidth, children: [previewLines.map((line, index) => (_jsx(Text, { color: theme.muted, wrap: "truncate", children: line }, index))), pastedText && pastedText.split(/\r?\n/).length > 3 ? (_jsx(Text, { color: theme.muted, children: "\u2026" })) : null, _jsx(Text, { color: theme.accent, children: t('input_press_enter') })] })) : null, _jsx(Box, { borderStyle: "single", borderColor: theme.inputBorder, paddingX: 1, width: fieldWidth, children: _jsxs(Text, { wrap: "truncate", children: [_jsx(Text, { color: theme.user, children: '> ' }), modePrefix ? (_jsxs(Text, { color: agentMode === 'yolo' ? theme.yoloAccent : agentMode === 'plan' ? theme.planAccent : theme.accent, children: [modePrefix, " "] })) : null, tag ? _jsx(Text, { color: theme.accent, children: tag }) : null, isEmpty && !tag ? (_jsx(Text, { color: theme.muted, children: placeholder })) : (_jsx(Text, { color: theme.user, children: displayTyped })), _jsx(Text, { color: theme.title, children: caretVisible ? '█' : ' ' })] }) })] }));
}
