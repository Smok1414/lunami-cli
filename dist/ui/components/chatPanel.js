...
export function ChatPanel(props) {
    ...
    const messageAreaHeight = Math.max(4, panelHeight - headerLines - pinnedFooterLines - paletteLines - activityLines - (hiddenMessageCount > 0 ? 1 : 0) - (hiddenMessageCountBottom > 0 ? 1 : 0));
    useEffect(() => {
    ...
    return (_jsxs(Box, { flexDirection: "column", width: width, height: panelHeight, borderStyle: borderStyle, borderColor: borderFlash ? theme.borderFlash : theme.border, paddingX: 1, paddingY: 0, children: [_jsx(Header, { sessionName: sessionName, cwdLabel: cwdLabel, hasContext: hasContext, theme: theme, width: contentWidth }), _jsx(Divider, { width: contentWidth, color: borderFlash ? theme.borderFlash : theme.border }), overlayOpen ? (_jsx(Box, { flexDirection: "column", height: pickerAreaHeight, overflow: "hidden", children: writeApproval ? (_jsx(WriteApprovalOverlay, { approval: writeApproval, theme: theme, height: pickerAreaHeight })) : helpOpen ? (_jsx(HelpOverlay, { theme: theme, height: pickerAreaHeight })) : modelPickerOpen ? (_jsx(ModalOverlay, { title: t('model_picker_title'), theme: theme, height: pickerAreaHeight, children: _jsx(ModelPicker, { groups: modelPickerGroups, search: modelPickerSearch, loading: modelPickerLoading, selectedIndex: modelPickerIndex, customMode: modelPickerCustomMode, customInput: modelPickerCustomInput, theme: theme }) })) : (_jsx(ModalOverlay, { title: apiSetupMode ? t('api_setup_title') : t('api_title'), theme: theme, height: pickerAreaHeight, children: _jsx(ApiPicker, { phase: apiPickerPhase, selectedIndex: apiPickerIndex, endpoint: apiPickerEndpoint, apiKey: apiPickerKey, activeField: apiPickerField, currentEndpoint: apiCurrentEndpoint, setupMode: apiSetupMode, theme: theme }) })) })) : (_jsxs(_Fragment, { children: [_jsxs(Box, { flexDirection: "column", height: messageAreaHeight, overflow: "hidden", marginBottom: 1, justifyContent: messages.length === 0 ? 'center' : 'flex-start', children: [hiddenMessageCount > 0 ? (_jsx(Text, { color: theme.muted, children: t('hidden_messages', hiddenMessageCount) })) : null, messages.length === 0 ? (_jsx(EmptyState, { theme: theme, height: messageAreaHeight, width: contentWidth })) : (messages.map((message) => (_jsx(MessageLine, { message: message, theme: theme, textWidth: textWidth, expanded: expandedToolIds.has(message.id), onToggleExpand: () => onToggleToolExpand(message.id), isStreaming: message.id === streamingMessageId && message.id === lastAssistantId, showTimestamp: message.kind === 'user' || message.id === lastAssistantId }, message.id)))), hiddenMessageCountBottom > 0 ? (_jsx(Text, { color: theme.muted, children: t('hidden_messages_bottom', hiddenMessageCountBottom) })) : null] }), isBusy ? (_jsx(Box, { flexDirection: "column", marginBottom: 1, children: _jsx(ActivityLog, { steps: activitySteps, theme: theme, fallbackLabel: toolLabel || t('agent_thinking') }) })) : null] })), paletteVisible && !overlayOpen ? (_jsx(CommandPalette, { commands: paletteCommands, selectedIndex: paletteIndex, query: paletteQuery, theme: theme })) : null, _jsxs(Box, { flexDirection: "column", flexShrink: 0, children: [_jsx(Divider, { width: contentWidth, color: theme.border }), _jsx(ModelBar, { modelLabel: modelLabel, agentMode: agentMode, theme: theme }), !overlayOpen ? (_jsx(InputBox, { typedText: input, pastedText: pastedText, boxWidth: contentWidth, theme: theme, agentMode: agentMode, placeholder: t('input_placeholder') })) : null, _jsx(ShortcutBar, { theme: theme }), _jsx(StatusBar, { status: status, tokenCount: tokenCount, theme: theme, hasContext: hasContext, toolLabel: toolLabel, toolChain: toolChain })] })] }));
}
function Divider({ width, color }) {
    return (_jsx(Text, { color: color, wrap: "truncate", children: '-'.repeat(Math.max(1, width)) }));
}
