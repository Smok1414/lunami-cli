...
export function CommandPalette({ commands: items, selectedIndex, query, theme }) {
    ...
    let flatIndex = 0;
    const borderStyle = prefersAsciiOutput() ? undefined : 'single';
    return (_jsxs(Box, { flexDirection: "column", flexShrink: 0, borderStyle: borderStyle, borderColor: theme.inputBorder, paddingX: 1, marginBottom: 1, children: [groupOrder.map((group) => {
                const groupItems = items.filter((c) => c.group === group);
                if (groupItems.length === 0) {
                    return null;
                }
                return (_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: theme.accent, bold: true, children: asciiFallback(`─ ${groupTitle[group]()} ─`, `- ${groupTitle[group]()} -`) }), groupItems.map((cmd) => {
                            const index = flatIndex;
                            flatIndex += 1;
                            const isSelected = index === selectedIndex;
                            const descParts = fuzzyHighlightParts(cmd.description, query.slice(1));
                            const prefix = isSelected ? asciiFallback('▸ ', '> ') : '  ';
