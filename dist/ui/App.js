...
import { changeApi, hasConfiguredApi, pingApiConnection, changeModel, getCurrentBaseUrl, getModelLabel, getProviderInfo } from '../llm.js';
...
export function App() {
    ...
    const applyMemoryState = useCallback((memory) => {
    ...
    const handleEvent = useCallback((event) => {
        ...
        if (event.type === 'assistant_delta') {
            setMessages((current) => current.map((message) => message.id === event.id ? { ...message, text: `${message.text}${event.delta}` } : message));
            return;
        }
    ...
    const runSessionCommand = useCallback(async (prompt) => {
    ...
    const toggleAgentMode = useCallback(() => {
        const next = agentMode === 'plan' ? 'auto' : agentMode === 'auto' ? 'yolo' : 'plan';
        setAgentMode(next);
    ...
    const toggleLastCollapsibleTool = useCallback(() => {
        for (let i = messages.length - 1; i >= 0; i -= 1) {
...
function trimHistorySafely(history, maxCount) {
...
function patchApprovalToolMessages(history, toolCallResultStr, toolCallId) {
