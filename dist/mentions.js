...
export function stripMentions(text) {
    return text.replace(MENTION_PATTERN, (_full, prefix) => prefix).replace(/\s{2,}/g, ' ').trim();
}
...
async function resolveMentionPath(inputPath) {
...
async function listDirectoryMentions(displayRoot, absoluteDir) {
    const results = [];
...
async function readMentionFile(displayPath, absolutePath) {
...
function buildMentionPreamble(resolved) {
