...
export class Reflector {
    reflectHeuristics(goal, steps, currentPlan) {
        if (steps.length === 0) {
            return {
                decision: 'retry',
    ...
    async reflectSemantically(goal, steps, currentPlan, llmCall, previousFeedback = '') {
        ...
        const formattedSteps = steps.length > 0
            ? steps
                .map((s, idx) => `Step ${idx + 1}: Tool "${s.toolName}" called with args: ${JSON.stringify(s.args)}\nOutput success: ${s.result.success}\nError: ${s.result.error || '<none>'}\nOutput preview: ${s.result.output.slice(0, 1000)}`)
                .join('\n\n')
...
FEEDBACK: [Specific next instruction for retry or adjust, otherwise omit]`;
    ...
    parseLlmReflection(output) {
        ...
        const decision = rawDecision || 'finish';
    ...
    classifyFailure(command, output) {
    ...
    hasLikelyCompletionSignal(goal, steps, currentPlan) {
        ...
        const completedAction = successfulSteps.some((step) => ['writeFile', 'file.write', 'execCommand', 'system.exec', 'generateProject', 'gitCommit'].includes(step.toolName));
