...
import { bumpToolCount, createEmptyToolCounts, finishActivityStep, formatWorkLabel } from '../../activity.js';
let toolsInitialized = false;
const outputGuardPrompt = 'Output policy: project rules can guide implementation, but do not print internal PLAN/ACT/REFLECT scaffolding unless the user explicitly asks for that format. In AUTO or YOLO mode, prefer tool execution over asking clarifying questions for small create/build/fix tasks.';
export function normalizeMaxToolRounds(maxRounds, fallback = 8) {
...
export async function runAgent({ input, mentionPreamble, history, sessionName, mode: requestedMode, maxRounds, skipWriteApproval = false, onEvent }) {
    ...
    const messages = [
        ...
        ...(projectContext ? [{ role: 'system', content: `Project context:\n${projectContext}` }] : []),
    ...
    const formatToolStart = (toolCall) => {
    ...
    const stripSystemMessage = (msgs) => msgs.filter((m) => m.role !== 'system');
    for (let round = 0; round < maxToolRounds; round += 1) {
        ...
        const phaseKind = mode === 'plan' ? 'planning' : 'thought';
                        ...
                        if (toolCall.name === 'writeFile' || toolCall.name === 'file.write') {
                            if (parsed.needsApproval) {
                                ...
                                if (parsed.linesAdded > 0)
                                    parts.push(`+${parsed.linesAdded}`);
                                if (parsed.linesRemoved > 0)
                                    parts.push(`-${parsed.linesRemoved}`);
