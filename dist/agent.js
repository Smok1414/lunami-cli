...
import { agentSkipsApprovals, getAgentMode, getCwd, getFullContext, setPendingApproval } from './state.js';
import { readFile, writeFile, previewWrite } from './tools/fs.js';
import { execCommand } from './tools/exec.js';
import { generateProject } from './tools/generate.js';
...
export async function runAgent({ input, mentionPreamble, history, sessionName, mode: requestedMode, skipWriteApproval = false, onEvent }) {
...
function getStringArg(toolCall, key) {
...
function getProjectTypeArg(toolCall, key) {
...
function getNumberArg(toolCall, key, fallback) {
    const value = toolCall.arguments[key];
    return typeof value === 'number' && Number.isFinite(value) ? value : fallback;
}
function stripSystemMessage(messages) {
    return messages.filter((message) => message.role !== 'system');
}
function formatLineDiff(added, removed, isNew) {
    if (isNew) {
...
function formatToolStart(toolCall) {
